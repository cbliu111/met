# code from
# Tang, Y., Weng, J. & Zhang, P. Neural-network solutions to stochastic reaction networks. Nat Mach Intell 5, 376–385 (2023).
...
class GRU(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.MConstrain = kwargs['constrains']
    ...
    def _forward_conserve(self, x, h, remain):
        ...
        if self.net_depth == 1:
            ...
            # If negative index, still make this normalization, but in log-prob does not take them
            normalizationfactor = torch.gather(temp, 1, remain.view(-1, 1).abs())
            expy = expy / normalizationfactor
            Aux = torch.arange(0, self.conservation + 1).repeat(remain.shape[0], 1).to(self.device,
    ...
    def log_prob(self, x):
        ...
        if self.net_depth == 1:
            ...
            h_init = torch.zeros(batch_size, self.net_width, self.net_depth, dtype=default_dtype_torch,
                                 device=self.device)
        epsilon = torch.log(torch.as_tensor(self.epsilon).to(self.device,
                                                             dtype=default_dtype_torch))  # -torch.as_tensor(30).to(self.device,dtype=default_dtype_torch)
        ...
        if self.conservation > 1:
            ...
            log_probTemp = y.gather(1, ids.view(-1, 1))[:,
                           0]  # #renormalized prob for those small number of this species to keep total number conservation
            Mask0 = remain < 0  # drop
            Mask1 = remain >= 0  # keep
        ...
        if self.reverse:
            ...
            ids_rev = x[:, -1].long()
            log_prob_rev[:, 0] = y_rev.gather(1, ids_rev.view(-1, 1))[:,
                                 0]  # y[:, 0] * mask[:, 0] + y[:, 1] * (1.0 - mask[:, 0])
        ...
        for i in range(1, self.n):
            ...
            if self.conservation > 1:
                ...
                if i == self.n - 1:
                    ...
                    log_prob[
                        ~Mask, i] = epsilon  # -torch.as_tensor(100).to(self.device)#-30*torch.ones(1).to(self.device)
        ...
        for i in range(1, self.n):
            ...
            if self.conservation > 1:
                ...
                if i < self.n - 1:
                    ...
                    samples[Mask, i] = torch.as_tensor(0).to(self.device,
                                                             dtype=default_dtype_torch)  # *log_prob[Mask, i]
                    samples[~Mask, i] = Temp[~Mask].to(
                        default_dtype_torch)  # -torch.as_tensor(100).to(self.device)#-30*torch.ones(1).to(self.device)
                if i == self.n - 1:
                    Mask = remain <= 0  # The configuration preserving conservation:Make its False prob to be zero in the end
                    samples[Mask, i] = torch.as_tensor(0).to(self.device,
                                                             dtype=default_dtype_torch)  # Temp[Mask].to(default_dtype_torch)#.long()#torch.as_tensor(0).to(self.device)#*log_prob[Mask, i]
                    samples[~Mask, i] = remain[~Mask].to(
                        default_dtype_torch)  # .long()#-torch.as_tensor(100).to(self.device)#-30*torch.ones(1).to(self.device)
