...
from met.hyperparameters import default_dtype_torch
from met.base import BaseModel
import numpy as np
...
class TraDE(BaseModel):
    ...
    def forward(self, x):
        ...
        x = self.fc_out(x)
        # If no number constrain, number constrain equals maximum boundary
        if self.MConstrain[0] == 0 or np.all(self.MConstrain == self.M):
            x_hat = F.log_softmax(x, dim=2)  # F.softmax(x, dim=2)
        else:  # Number constrain
            x_hat = torch.ones_like(x) * (-100)  # exp(-100) is about 3e-44 probability
            index1 = np.arange(self.n)[self.MConstrain < self.M]  # find out the species with constrains less than M
            index0 = np.arange(self.n)[self.MConstrain == self.M]  # find out the species with constrain equals to M, by default all species with constrains equal to M
            index11 = self.MConstrain[index1[0]]  # find out the exact number of constrain of the species with constrains less than M
            index00 = self.MConstrain[index0[0]]  # find out the exact number of constrain of the species with constrains equal to M
            # normalize by considering the number constrains
            # normalization is done within each species, with range [0, constrain] for each species
